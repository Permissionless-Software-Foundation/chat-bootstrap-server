...
const queryControl = async (from, to, channelName, dbName, dbID) => {
   ...
   */
    ...
    try {
        ...
        //encrypting entry 
        const entryEncrypted = await encrypt(JSON.stringify(entry), ENCRYPTION_PASSWORD)
        //adding to db
...
const generatePassword = async () => {
    ...
    let pass = "";
    for (let i = 0; i < long; i++) pass += charts.charAt(Math.floor(Math.random() * charts.length));
    return pass;
